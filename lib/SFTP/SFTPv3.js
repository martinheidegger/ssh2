...
function fastXfer(src, dst, srcPath, dstPath, opts, cb) {
  ...
  src.stat(srcPath, function(err, attrs) {
    if (err) return onerror("Error occured while accessing the local file: "+err);
    fsize = attrs.size;

    src.open(srcPath, 'r', function(err, sourcefd) {
      if (err) return onerror("Error occured while opening the local file: "+err);
      srcfd = sourcefd;

      dst.open(dstPath, 'w', function(err, destfd) {
        if (err) return onerror("Error occured while opening the remote path: "+err);
        dstfd = destfd;

        if (fsize <= 0) return cb();
        function onread(err, nb, data, dstpos, datapos) {
          if (err) return onerror("Error occured while reading local file: "+err);

          dst.write(destfd, data, datapos || 0, nb, dstpos, function(err) {
            if (err) return onerror("Error occured while writing remote file: "+err);

            if (--reads === 0) {
              if (total === fsize) {
                dst.close(destfd, function(err) {
                  dstfd = undefined;
                  if (err) return onerror("Error occured while closing remote file: "+err);
                  src.close(sourcefd, function(err) {
                    srcfd = undefined;
                    if (err) return onerror("Error occured while closing local file: "+err);
                    cb();
                  });
                });
              } else
